...
public class MySpringBootRouter extends RouteBuilder {
    ...
    private final String basePath = System.getenv().getOrDefault("BASE_PATH", "http://localhost:8182").trim();
    ...
    private final boolean allowCredentials = Boolean
            .parseBoolean(System.getenv().getOrDefault("CORS_ALLOW_CREDENTIALS", "false"));

    private final String allowMethods = System.getenv().getOrDefault("CORS_ALLOW_METHODS",
            "GET,POST,PUT,DELETE,OPTIONS");

    private final String allowHeaders = System.getenv().getOrDefault("CORS_ALLOW_HEADERS",
    ...
    public void configure() {
        ...
        from("netty-http:http://0.0.0.0:8081?matchOnUriPrefix=true&httpMethodRestrict=GET,POST,PUT,DELETE,OPTIONS")
                ...
                .process(e -> {
                    ...
                    // If BASE_PATH mistakenly contains a path, keep only scheme://host:port
                    int schemeIdx = bp.indexOf("://");
                    if (schemeIdx > 0) {
                        int firstSlashAfterHost = bp.indexOf("/", schemeIdx + 3);
                        if (firstSlashAfterHost > 0) {
                            bp = bp.substring(0, firstSlashAfterHost);
                        }
                    }
                ...
                // IMPORTANT: use netty-http as the producer component (since you already have
                // it)
                .toD("netty-http:${exchangeProperty.proxy.targetUrl}"
    ...
    private void addCorsHeadersIfAllowed(Exchange e) {
        ...
        if (origin == null || !Boolean.TRUE.equals(allowed))
            return;
